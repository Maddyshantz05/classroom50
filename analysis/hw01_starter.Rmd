...
3. If not already complete, fill in your information in the Student Info spreadsheet under the Canvas Collaboration site.  

## Analysis Elements: {-}
Rename the starter file under the analysis directory as `hw01_yourname.Rmd` and use it for your solutions.   
...
- Apply concepts and methods from DATA-412/612.   
...
4. Completed my information in the Student Info spreadsheet on Canvas collaborations

# College Scorecard

The data folder contains "college_scorecard_extract_ 2021-01-19.csv", a subset of the data in the [College Scorecard](https://collegescorecard.ed.gov/data/) database as of January 19, 2021. These data contain information on colleges in the United States. The variables include:
...
1. Use a relative path and a readr function to load the data from data/college_score_200601.csv into a tibble. Note, this data uses NULL instead of `NA` so convert NULL to `NA` while reading in.
...
4. How is average faculty salary associated with the median earnings of students ten years after initial enrollment? 

  - Choose response and explanatory variables based on a reasonable hypothesis. 
  - Create an appropriate plot to assess for a linear relationship with smoother
  - Use a ggplot2 function to try to reduce overplotting.
  - Interpret the plot to justify your answer. 
...
2. These data are messy. The observational units in `fert`, `life`, and `pop` are locations in space-time (e.g. Aruba in 2017). Recall tidy data should have one observational unit per row.  
  ...
  - Use an argument so the readr function ensures the variable for `year` is a numeric.  
...
6. Make a bar plot of population vs region for the year 2010.  
  + Order the bars on the $y$-axis in **increasing** order of population. 
  + Your final plot should look like this:
